...
(function () {
  ...
  /* (se retiró requestPersist — ver la nota 🕊 en boot(): nunca pedimos el
     permiso de almacenamiento persistente; las 4 capas bastan y sobran)  */
  ...
  async function boot() {
    ...
    writeAll();
    /* 🕊 NADA de navigator.storage.persist(): ese permiso de «almacenamiento
       persistente» asusta a la gente y no aporta nada visible — la identidad
       ya vive en 4 capas (localStorage + IndexedDB + Cache + cookie) y los
       datos se guardan exactamente igual sin pedirle nada al visitante. */
    renderChip();
    applyRole();
  ...
  /* firma un texto suelto con tu clave del dispositivo (prueba de admin
     para cosas que viven en el servidor, como los ajustes del chat) */
  async function signText(text) {
    try {
      ...
      if (!rec) return null;
      const priv = await crypto.subtle.importKey('jwk', rec.jwkPriv, { name: 'ECDSA', namedCurve: 'P-256' }, false, ['sign']);
      const sig = await crypto.subtle.sign({ name: 'ECDSA', hash: 'SHA-256' }, priv, new TextEncoder().encode(String(text)));
      return bytesToB64(new Uint8Array(sig));
    } catch (e) { return null; }
  }
